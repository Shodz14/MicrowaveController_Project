...
{
	char SW_3;
	SW_3 = (GPIO_PORTE_DATA_R & 0X20); //reading SW3 (door position) PortE_5
	return SW_3;
}
...
int main()
{
	...
	switch(key){
		...
		// Display "PopCorn" for 10 sec 
			door = check_door();				//putting return of func into var
		  if(door == 0) //door closed = 0
		  {  
		  	LCD_Command(Clear_Display); //Clear please close the door if closed before 5 sec
		  	LCD_Countdown(60);					// Countdown 60 sec
